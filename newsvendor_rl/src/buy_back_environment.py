...
l: price
l+1: cost
l+2: penalty for lost sale (civ)
l+3: mean demand (assuming Poisson distribution)
...
class NewsVendorGymEnvironment(gym.Env):
    ...
    def __init__(self, config={}):
        ...
        space_low += 5 * [0]
        space_high += [self.max_value, self.max_value, self.max_loss_goodwill, self.max_mean]
    ...
    def reset(self):
        ...
        self.state[self.inv_dim + 2] = loss_goodwill
        self.state[self.inv_dim + 3] = mean_demand
    ...
    def break_state(self):
        ...
        k = self.state[self.inv_dim + 2]
        mu = self.state[self.inv_dim + 3]
        return inv_state, p, c, k, mu

    def step(self, action):
        done = False
        inv_state, p, c, k, mu = self.break_state()
        ...
        reward = sales_revenue - purchase_cost- penalty_lost_sale
