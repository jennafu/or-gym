...
class BuybackContractEnv(gym.Env):
    '''
    Multi-Period Buyback Contract Mechanism

    The Multi-Period Buyback requires meeting stochastic demand by having sufficient
    ...
    holding costs associated with holding unsold inventory, unsold
    inventory will be bought back by suppliers at the end of each period.

    Observation:
        Type: Box 
        State Vector: S = (p, c, h, mu, x_l, x_l-1)
        p = price
        c = cost
        h = holding cost
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.observation_space = spaces.Box(
            ...
            high=np.array(
                [self.p_max, self.p_max, self.h_max, self.mu_max] +
    ...
    def _STEP(self, action):
        ...
        buyback_amount = excess_inventory * self.buyback
        reward = sales - purchase_cost - holding_cost + buyback_amount
    ...
    def _RESET(self):
        ...
        self.salvage = max(1, np.random.rand() * self.cost)
        self.buyback = np.random.rand() * (self.price - self.salvage)
